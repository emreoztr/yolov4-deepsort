# yolov4-locking-with-csrt-tracker
## My Part of the Readme

From UAV Competition that I joined in my second year.
Forked the base part of this project from [theAIGuysCode](https://github.com/theAIGuysCode)

Main purpose of this project is locking an UAV with a camera and counting the time of locking. It uses CSRT algorithm implemented in OpenCV for Object Tracking, not using the DeepSort from the implementation of [theAIGuysCode](https://github.com/theAIGuysCode) . Program stops using Object Detection if the object gets in the locking area and starts using CSRT algorithm for tracking, this increases the performance of the program while tracking and makes it more realible in the locking stage.

## Demo of Locking and Searching Stages
<p align="center"><img src="data/helpers/uav_demo.gif"\></p>
...
To implement the object tracking using YOLOv4, first we convert the .weights into the corresponding TensorFlow model which will be saved to a checkpoints folder. Then all we need to do is run the object_tracker.py script to run our object tracker with YOLOv4, CSRT and TensorFlow.
```bash
